...
class GiniIndex
{
    ...
    {
        BufferedReader br=new BufferedReader(new FileReader("C://Users/it/Desktop/data.csv"));
    ...
    public static double findAttributeGini(String [][] array,int col_no)
    {
        ...
        for(Map.Entry<String,Map<String,Integer>> e1:m.entrySet())
        {
            ...
            for( i=0;i<l1.size();i++)
            {
               double ans=l1.get(i)*l1.get(i);
               attr_gini[k++]+=ans;
            }
            for(i=0;i<k;i++)
            {
               attr_gini[i]=1-attr_gini[i];
               attr_gini[i]=(1.0*tot/array.length)*attr_gini[i];
            }
